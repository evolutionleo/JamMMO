...
globalvar ARRAY_LOOP; // edit this to exit the loop
ARRAY_LOOP = 1;
...
function Array() constructor {
	...
	static findAll = function(val) {
		...
		findAnswer(function(x, pos, struct) {
			var val = struct.val;
			var ans = struct.ans;
	...
	///@description	loops over the Array and returns the value when your function returns anything but undefined
	...
	static __partition = function(l, r) {
		var i = l-1;
		var j = l;
		var piv = get(r);
	...
	static quickSort = function(l, r) { // including r
		if is_undefined(l)
			l = 0
		if is_undefined(r)
			r = size-1
